...
class DroidDataset(Dataset):
    ...
    def __init__(
        ...
        self.root = Path(data_path) / f"{dataset_name}_memmap_h{horizon}_{img_size[0]}x{img_size[1]}"
        ...
        cam_key = self.camera
        self.obs = np.memmap(self.root / f"obs_{cam_key}.dat", dtype="uint8", mode="r",
                             shape=(self.N, 3, *self.img_size))
        self.context_obs = np.memmap(self.root / f"context_obs_{cam_key}.dat", dtype="uint8", mode="r",
                                     shape=(self.N, self.horizon, 3, *self.img_size))
        self.context_acts = np.memmap(self.root / "context_acts.dat", dtype="float32", mode="r",
                                      shape=(self.N, self.horizon, self.action_dim))

    def _resize(self, img):
        return np.array(Image.fromarray(img).resize(self.img_size[::-1], Image.Resampling.BILINEAR))

    def _build_memmaps(self, data_path):
        print("[DroidDataset] Building memmaps from TFDS…")
        episodes = tfds.as_numpy(tfds.load(self.dataset_name, split='train', data_dir=data_path, download=False))
        ...
        for episode in episodes:
            ...
            for step in episode["steps"]:
                obs_all = {cam: step["observation"][cam] for cam in self.SUPPORTED_CAMERAS}
                act = step["action"]
        ...
        # Save everything
        for cam in self.SUPPORTED_CAMERAS:
            np.memmap(self.root / f"obs_{cam}.dat", mode="w+", dtype="uint8",
                      shape=(N, 3, H, W))[:] = np.stack(obs_by_cam[cam]).transpose(0, 3, 1, 2)
            np.memmap(self.root / f"context_obs_{cam}.dat", mode="w+", dtype="uint8",
                      shape=(N, self.horizon, 3, H, W))[:] = np.stack([
                          x.transpose(0, 3, 1, 2) for x in ctx_obs_by_cam[cam]
                      ])

        np.memmap(self.root / "context_acts.dat", mode="w+", dtype="float32",
                  shape=(N, self.horizon, action_dim))[:] = np.stack(ctx_acts)
    ...
    def __getitem__(self, idx):
        ...
        context_obs = torch.from_numpy(self.context_obs[idx].astype(np.float32) / 127.5 - 1.0)
